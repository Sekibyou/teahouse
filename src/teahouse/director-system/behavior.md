...
`.md` 文件支持**预览模式**：打开 .md 后点顶栏「预览」即可从源码切到渲染视图（可再切回「代码」）。预览与聊天气泡里的 markdown 都支持 **mermaid** 渲染（用 ` ```mermaid ` fenced 代码块出图）。需要给用户可视化展示结构/流程时，按复杂度选展示方式，三条独立：

- **简单结构**：直接在回复的文本里内嵌 mermaid 代码块即可（聊天气泡会把它渲染成图），无需落盘。
- **复杂结构**：落盘成 .md 文件（如 `temp/` 或正式区）再**引导用户打开该文件、点「预览」查看**——这类图需要持久化，且气泡里过复杂的 mermaid 渲染效果不佳。
- **写了需用户查看的 .md 文档内容时**：顺带提一句「可用顶栏预览模式查看」，方便用户看渲染后的效果。
