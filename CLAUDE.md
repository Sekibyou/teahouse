...
> **分步参数**：`build_release.py [--check] [--frontend] [--backend] [--assets] [--verify] [--zip]`——带任一参数**只跑对应步骤**；无参数 = 全跑。**⚠️ `--zip` 只是"用当前 `dist/Teahouse` 直接打包"，不重编前端/后端**——改过代码后必须先 `--frontend`/`--backend`（或干脆无参数全跑）再 `--zip`，否则打出来的是旧代码。发布走下方"发布流程"用无参数全跑即可。`--zip` 额外产出 `dist\Teahouse-<ver>-Windows-with-git.zip`（含 git，新用户）与 `dist\Teahouse-<ver>-Windows.zip`（不含 git，更新包），版本号唯一权威 = `pyproject.toml`（两态解析：源码态 `_version.py` 实时读 pyproject；打包态 build_release 写入 gitignore 的 `_version_inject.py` 供冻结代码读。**改版本号只改 pyproject 一处**，前端经 `GET /v1/status` 拉后端权威版本显示与比对，`package.json` 的 version 不再作权威。详见 build/README_BUILD.md「版本号管理」）。
