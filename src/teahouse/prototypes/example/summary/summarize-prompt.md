...
4) Write 流水账到 summary/sum-A-B.md。注意：每次总结最多覆盖 10 章，超过 10 章需拆分成多个 sum-*.md 文件（如 sum-1-10.md、sum-11-20.md…），每个单独提交；
5) 用 GitCommit(type="summary", start=A, end=B, paths=["settings/dyn_settings", "generate-config", "summary"], message="简短描述") 提交。若拆分为多个范围，逐个提交。提交前用 GitStatus / GitDiff(staged=true) 自查本次 stage 的正是总结自己的改动（别把主会话未提交的楼层/变量卷进来）。
...
**维护说明**：以下内容与创作强相关，按实际实例修改：
- 「本次总结要维护的动态设定」：列出 `settings/dyn_settings/` 下需要总结时更新的文件
- 「本次总结要维护的变量」：列出需要在总结时按剧情更新的变量
- 「静态文件检查」：列出总结时需顺带检查的静态切片源

【本次总结要维护的动态设定】
- settings/dyn_settings/角色动态.md（覆盖式更新：当前心理/所在地/现场状态）
...
【静态文件检查】
- 切片源是否需补区块（settings/static_settings/好感度阶段提示词.md 若出现新档位）