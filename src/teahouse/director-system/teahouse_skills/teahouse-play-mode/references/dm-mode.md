...
- **状态与存档**：变量用 `SetRuntimeVar` 维护、`GetRuntimeVars` 读；阶段性进展在合适时机 `GitCommit` 存档。**DM 模式下每次 `GitCommit` 都会顺带把权威变量快照推进到当前状态**（小说模式靠转正推进，DM 没有转正），所以存档点即变量快照点——提交之后即使切分支/丢弃改动，变量也回到那次的存档，而不是开局。反过来说：**没提交的变量改动不在快照里**，要靠存档保住的大进展别忘了提交。
