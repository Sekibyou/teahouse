...
| `{{glob:模式}}` / `{{glob:模式:lastN}}` | 按 glob 匹配多个文件（按文件名排序）；`:lastN` 按名字中的数字段降序保留最近 N 个**楼层**（正式稿优先于草稿）。例：`{{glob:runtime/floors/floor-*.md:last10}}`。解析结果各文件以 `── file: <路径> ──` 开头、空行分隔拼接 |

### 修饰符（可叠加）

| 修饰 | 作用 |
|---|---|
| `:行段` | 行范围（行号 1 起、含端点）：`{{a.md:10-30}}` 第 10~30 行、`{{a.md:6}}` 第 6 行、`{{a.md:6-}}` 第 6 行到文末、`{{a.md:-6}}` 开头到第 6 行 |
...
- **⚠️ `between/and` 结果含锚点本身**，锚点若嵌在某行中间，首尾会各带半截残余（如 `...max:mid -->` / `<!-- section:tail`）。想让两种写法结果一致，就让切片草稿里的锚点**独占一行**（如单独一行写 `@@A@@`），或干脆只用行级的 `from/to` + `:行段`。
- **写错不静默**：行段（`:6` 拼成 `:abc`）或修饰符（`from=` 拼成 `form=`、漏了引号）若无法解析，严格面报错、宽松面整片原样保留（见下「失败行为」）——**绝不会悄悄换成整份文件**。
