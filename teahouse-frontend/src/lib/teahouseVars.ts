...
/** 断言 name 无空白、无冒号、无 @ —— 与后端 validate_var_name 一致（空白破坏 ${...} 标识符；冒号让裸 ${...} 被判定层误判；@ 是 ${@name} 显式指令的保留前缀）。 */
function validateName(name: unknown): string | null {
  if (typeof name !== "string") return "name 必须是字符串"
  if (name.trim() === "") return "name 不能为空"
  if (/\s/.test(name)) return `变量名「${name}」禁止含空白字符`
  if (name.includes(":")) return `变量名「${name}」禁止含冒号「:」`
  if (name.includes("@")) return `变量名「${name}」禁止含「@」（\${@name} 显式指令保留前缀）`
  return null
}
