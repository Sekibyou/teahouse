...
| `ui_js` | 嵌入 srcdoc `<script>`；新增的通过 SSE `output.append` 事件动态 eval | 多个并存，追加式 | 嵌入 srcdoc `<script>` | 悬浮组件：状态栏、背包面板、角色卡、计数器、输入框 |
...
<script>scene.js</script>        ← 2. 次执行：此时 DOM 可能还在 loading
<script>color-button.js</script>  ← 3. 最后执行：ui_js 块，此时 #teahouse-ui-layer 可能还不存在！
```
...
**结论**：`ui_js` 和 `scene_js` 必须使用 `window.registerUI()` 挂载元素，不能直接操作 `#teahouse-ui-layer`。`registerUI` 内部有排队机制——如果 UI 层还没创建，它会先把元素放入 `uiQueue`，等 `DOMContentLoaded` 触发后再 flush。
...
12. **共享状态挂载到 `window.Teahouse` 上并带事件通知**：状态变更方调用 `window.Teahouse._emit('xxx.change', data)` 通知所有订阅者，订阅方通过 `window.Teahouse.on('xxx.change', callback)` 响应。这样做的好处是 ui_js / scene_js / bootstrap 任意三方之间无需知道彼此存在即可同步。
