...
包含两个部分：**BBCode 渲染**（怎么写内容）与**文本样式规则**（怎么管理符号着色规则）。

当输出内容只使用纯 Markdown 或纯 HTML 标签时，无需本文件。当需要以下任一特性时必须读：
- BBCode 标签（格式化或动画特效）
...
```
原始文本
  → 0. 预处理：禁用 Markdown 列表语法（行首 `1.` / `-` / `*` 后插零宽空格）、剥掉行首 4 空格/tab 缩进
  → 1. BBCode 解析（[b][i][shake] 等标签 → HTML）
  → 1.5 多行 BBCode span 内部的换行 → <br>
  → 2. 文本样式规则着色（符号对如《》、引号等 → 包裹 CSS）
  → 3. HTML 块元素（<details>）内部的 Markdown 预解析
  → 4. Markdown 解析（GFM：标题、列表、表格、代码块等）
  → 5. 渲染为 HTML 插入沙盒
```
...
2. **BBCode 标签可以跨行**，`[glow]...[/glow]` 可以包裹多个段落。BBCode 生成的是内联 HTML，多行 `<span>` 内部的**单个换行会被自动转成 `<br>`**（渲染管线对多行 span 做 `\n` → `<br>`，且 marked 开着 `breaks`）；要分段仍建议用空行（两个换行）。
3. **不要在 BBCode 标签内部嵌套同类型标签**，但可以嵌套不同类型。
4. **HTML 块元素（如 `<details><summary>`）内部的 Markdown 会被解析**。

特效参数均可省略，使用默认值。**特效标签的参数只认 `key=value` 形式**（多个用空格分隔）；`[shake=0.5s]` 这种单参数简写**不会被解析**——参数被整段丢弃，全部回落到默认值（`[shake=0.5s]` 看似生效只是因为它恰好等于默认值，写 `[shake=1s]` 依旧是默认的 0.5s）。
- 推荐：`[shake rate=0.5s level=3]文字[/shake]`
- 省略参数：`[shake]文字[/shake]`（全用默认值）
- 例外：标准格式化的 `[color=#ff0000]`、`[size=20]` 惯用 `=` 单参数形式（**推荐写法**）。渲染器对这两个标签也接受空格形式 `[color #ff0000]`，但没必要刻意用。
...
- **BBCode 不能整块包裹列表**。列表语法本身已被禁用（见下条），`[bounce]01. 第一项\n02. 第二项[/bounce]` 不会生成 `<ol>`，但多行内容会被裹进一个内联 `<span>`、丢掉块级排版。需要给列表项加特效，请**逐项包裹**。
- **不建议 BBCode 与 Markdown 内联混用**。`[rainbow]**文字**[/rainbow]` 里的 `**` **会**被 marked 照常解析为加粗（BBCode 转出的内联 `<span>` 不阻断行内 Markdown）。但为可读一致，BBCode 内建议统一用 `[b]` 加粗、`[i]` 斜体。
- **默认字号是 16px**。`[size]` 的单位是 px，`[size=20]` = 20px，不要用 5、6 这种小数值，会导致文字不可读。建议取值范围：14~32。
- **Markdown 表格内部可以使用 BBCode**。BBCode 在 Markdown 之前解析，表格单元格里的 `[b]` 此时已变成 `<strong>`，markdown 表格渲染时正常保留。
...
**六个字段全部必填**（读取端不做默认值补全）：漏写 `enabled` 的规则会被当成**禁用**、静默不生效（判断取的是真值）；漏写 `order` 会让排序比较出 `NaN`、顺序不再可靠。
