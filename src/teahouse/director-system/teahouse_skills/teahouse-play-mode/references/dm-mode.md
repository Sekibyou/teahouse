...
**预设对话推荐顶层 `user:` / `assistant:`**（单轮 few-shot 就够），不要用 `messages:` 列表——那要多一层列表缩进，更容易写出 YAML 语法错，只有多轮 few-shot 才值得用它。

**⚠️ 踩坑：`|` 块标量内每行缩进必须一致。** 块标量的缩进以块内**第一条非空行**为准，之后某行只要比它浅一格，就被判定"掉出块标量"，整个文件随即解析失败（不是只丢这一行）。这种错以前会让 DM 会话**静默死掉**——前端既不报错也不结束，只停在"运行中"，作者在 VPS 上完全看不出原因。现在会明确报错：会话里出现一条错误记录 + 前端提示，含出错文件、行号、出错行原文与修复提示。

**改完 `dm.yaml` 先体检**：在导演会话里调 `CheckDMConfig` 工具，它把「会崩的错」（YAML 语法 / 顶层不是映射 / `messages:` 不是列表 / role 非法）与「静默失效的错」（切片解析不到、变量未定义、顶层键拼错、`system:` 为空、每轮动态值写在 `user_tail` 之外）一起报出来。语法坏到起不来时 DM 自己没法自查，所以这个入口在导演侧。
...
- **`EndTurn()`** 宣告**本轮**结束——后端见到它就停止本轮，不再多问一次（只结束本轮，会话照常存活，玩家下次发言照接）。**扮演回合必须用它收尾**：那条通道禁止 DM 写正文（见第五节），于是本轮以工具调用结束，后端无从判断 DM 是否还想继续，只能再发一次**只有输入、没有输出**的请求去问；`EndTurn` 省掉这趟空转（一次上下文费用 + 一次往返延迟）。放在本轮**最后一个**工具调用之后。它与 `EndSession` 不是一回事——后者是受限子会话用来结束**整个会话**并通知父会话的，DM 没有这个工具。
...
| 通道 | 玩法侧入口 | 控制台侧入口 | 去向 |
|---|---|---|---|
| **扮演**（剧情内） | 沙盒输入 `Teahouse.sessionSend('dm', text)`（参考渲染器里默认模式） | `/say 内容` | 进 `dm-output.jsonl`（开新批次）再交给 DM；DM 这一回合只准调工具、不写正文，故须以 `EndTurn` 收尾 |
| **局外** | 沙盒输入 `Teahouse.sessionSendOoc('dm', text)`（参考渲染器的「局外」开关） | DM 控制台输入框（默认语义） | 只进会话、**不进** dm-output；DM 可以回正文（只在 DM 栏可见），以正文自然收尾，无需 `EndTurn` |
