...
**打包之前，主动询问用户是否要为原型提供封面图**，不要默认跳过。

建议给用户的封面图规范（当用户愿意提供时）：

**支持的文件名**（放实例根目录，后端按此顺序选取，先到先得）：
- `cover.jpg`
- `cover.jpeg`
- `cover.png`
- `cover.webp`
...
- 不需要透明通道的封面用 JPEG/WebP 即可，别用 RGBA PNG（一张 1536×2752 的噪声 PNG 可达 9MB+）。

**尺寸建议**：
- 宽度 **1536px** 内即可（封面在瀑布流/缩略图展示，过大无收益）。
- 高度随画面比例（常见竖封约 9:16）。
...
**确认动作**：
- 用户提供封面图后，把文件放到实例根目录，用 `cover.*` 命名，并向用户确认。
- 若用户表示不需要封面图，可跳过（封面缺失只影响原型列表的显示，不影响导出）。
