...
**`${!-- ... --}`** 是写给作者/导演的**元信息注释**，写在与导演相关的设定文件里（`teahouse.md`、`static_settings/`、`dyn_settings/` 等），解析时被**剥除（替换为空）**：
...
- **语法规则**：`${` 后必须紧跟 `!-- `（`!--` 后一个空格），空格是与真实变量的关键区分（变量名禁止空白）。注释正文**可含任意字符，包括裸 `}`**，在第一个 `--}` 处结束。找不到 `--}` 则整段原样保留。
- **转义**：`\${!-- ... --}` 会原样显示成 `${!-- ... --}`（不剥除、不回显），用于要给正文看到注释文字的罕见场景。
- **不要用注释包住需要正文 AI「直接采用值」的内容**——它们会被剥掉，正文看不到。

示例（写进 `dyn_settings/characters.md`）：

```
金：12 ${!-- 角色金币数，会随剧情变动，总结时保留为字面量以便更新，勿写死 --}
