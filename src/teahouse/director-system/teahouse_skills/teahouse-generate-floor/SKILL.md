...
2. **编辑配置文件**（使用 Edit 工具进行精确修改）：
   ...
   - 在 user 消息中用 `${name}` 变量字面量引用当前变量值（如 `${金币}`、`${主角名}`、`${修为}`），Generate 发送给正文 AI 前会统一展开为值；也可用 `{{.teahouse/dyn_settings/xxx.yaml|...}}` 切片注入相关动态设定。**`${ }` 本身就代表「环境」**：括号内是变量命名空间，变量名（或条件表达式）一律**裸写**，无需再加任何包裹——无论是 `${user}` 这种普通变量、还是 `${ if 好感度 >= 80: return ... }` 这种条件块，里面的 `user`、`好感度` 都是直接写的裸变量名
   - **进阶：条件切片**——当需要"按变量值从几段里就地选一段灌给正文"（如骰子分档、好感度阶梯提示词）时，用 `${ if ...: return ... }` 代码块在解析阶段命中一段，未命中的分支不进入上下文。同属 `${ }` 环境：条件里引用的变量名（如 `好感度`）同样裸写只命中当前档位即可，不必把整段阶梯规则全发给正文。示例（每档一份独立设定文件，命中即物化整段）：
     ```
     ...
     **YAML 引号与转义（实践中最容易翻车的一层）**：Generate 配置是 **YAML 文件**，占位符/条件块是写在这个文件里的字符串——YAML 的引号规则会**先于**占位符解析生效，内嵌引号常在这里二次破坏。三句实战经验：
     ...
     - **占位符内嵌引号锚点会引号大战**：`{{file|from="..."}}` 这种锚点自带宽引号，再嵌进 `return "{{...}}"`，就出现引号里套引号——YAML 解析 + `{{}}` 展开两轮都容易炸。规避：**把含引号锚点的切片拆成独立的设定文件，块内 `return` 只写简单占位符**（如裸文件名 `{{room1.md}}`，或短字符串拼结），绝不把 `from="..."` 这类带引号锚点写进条件块的返回值里。阶段切片（如分档）拆成 4 个独立文件，每档 return 一个干净文件名，比在一个块里拼引号锚点稳得多。
