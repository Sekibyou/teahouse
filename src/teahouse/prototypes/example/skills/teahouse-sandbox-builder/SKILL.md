...
|---|---|---|---|---|
| `bootstrap_js` | append 后立即注入 iframe 并执行 | 全局唯一 | 嵌入 srcdoc `<script>` | 沙盒基础设施：第一个注入 srcdoc 的脚本，负责事件监听、渲染循环、Teahouse API |
| `ui_js` | 嵌入 srcdoc `<script>`；新增的通过 SSE `output.append` 事件动态 eval | 多个并存，追加式 | 嵌入 srcdoc `<script>` | 悬浮组件：状态栏、背包面板、角色卡、计数器、输入框；支持动态注入/移除 |
...
<script>color-button.js</script>  ← 2. 最后执行：ui_js 块，此时 #teahouse-ui-layer 可能还不存在！
```
...
这意味着 `bootstrap.js` 本身的 IIFE 执行 ≠ DOM 已就绪。bootstrap 内嵌的 `setTimeout(createColorButton, 100)` 能工作是因为延时跳过了 DOM 构建阶段。

**结论**：`ui_js` 必须使用 `window.registerUI()` 挂载元素，不能直接操作 `#teahouse-ui-layer`。`registerUI` 内部有排队机制——如果 UI 层还没创建，它会先把元素放入 `uiQueue`，等 `DOMContentLoaded` 触发后再 flush。
...
获取所有活跃输出块的摘要列表。返回的每个元素包含 `uuid`、`label`、`note`、`content_type`、`content`（模板字符串，前端解析）。
...
获取单个输出块的完整数据（含 content 模板字符串）。

```js
const block = await Teahouse.getOutputBlock("some-uuid")
const html = await Teahouse.renderRichText(block.content)
```
...
| 事件 | payload | 触发时机 |
|---|---|---|
| `output.append` | `OutputBlock`（含 content 模板字符串） | 导演 Output(mode="append") 后 SSE 推送 |
| `output.replace` | `OutputBlock`（含 content 模板字符串） | 导演 Output(mode="replace") 后 SSE 推送 |
...
**自动排队机制**：如果调用时 `#teahouse-ui-layer` 还没创建（DOM 未就绪），元素会被放入内部队列，等 UI 层就绪后自动挂载。这保证了 ui_js 在任何执行时机下都能安全注册。
...
Read .teahouse/output-blocks.jsonl       → 查看活跃输出块
```
...
5. 默认渲染逻辑 — 获取 ep 块、调用 `renderRichText()`、渲染为气泡
...
(function() {
  ...
  if (uiLayer) {
    ...
    // ui_js 可能在 DOM 就绪前执行，直接用 registerUI（内部会排队）
    window.registerUI('statusbar', bar)
  }
...
- 修改 bootstrap.js 后，前端会自动读取最新文件内容并重建 srcdoc——这意味着所有 DOM 状态丢失
- 修改 ui_js 或 css，前端也会重建 srcdoc（通过检测 uuid 变化）——同样会丢失 DOM 状态
- 修改 rich_text 通过 replace 推送，沙盒内的 Teahouse.on("output.replace") 事件会触发，由沙盒脚本负责原地更新
...
3. **共享状态通过 `window.Teahouse` 暴露**：`window.Teahouse._colorState`、`window.Teahouse._pageState` 等，不要用零散的 `window._teahouse_*` 全局变量
4. **跨组件通信通过事件**：`window.Teahouse._emit('color.change', data)` + `window.Teahouse.on('color.change', callback)` 实现松耦合
5. **`renderRichText` 返回的 HTML 用 `<hr>` 分隔段落**：可据此切片为气泡
6. **fixed 定位的 UI 组件 z-index 分层次**：topbar ~200、UI 层 ~100、panel ~200、input ~300
7. **不要在沙盒内写 ES6+ 语法**：`let`、`const`（IIFE 内闭包的局部 `const` 在现代浏览器中安全，但为兼容性最好用 `var`）、`=>`、模板字符串、Promise 链之外的 async/await 都不安全
...
11. **ui_js 必须通过 `window.registerUI(label, element)` 挂载 UI 元素**：不要直接 `getElementById('teahouse-ui-layer').appendChild()`，因为 ui_js 脚本在 srcdoc 中可能先于 bootstrap 的 DOMContentLoaded 回调执行，此时 `#teahouse-ui-layer` 还不存在。bootstrap 的 `registerUI` 会在 UI 层未就绪时自动排队，就绪后 flush。直接操作 DOM 会导致元素静默丢失。
12. **共享状态挂载到 `window.Teahouse` 上并带事件通知**：状态变更方调用 `window.Teahouse._emit('xxx.change', data)` 通知所有订阅者，订阅方通过 `window.Teahouse.on('xxx.change', callback)` 响应。这样做的好处是 ui_js / bootstrap 任意方之间无需知道彼此存在即可同步。
