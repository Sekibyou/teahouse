...
> **分步参数**：`build_release.py [--check] [--frontend] [--backend] [--verify] [--zip]`——带任一参数**只跑对应步骤**；无参数 = 全跑。**⚠️ `--zip` 只是"用当前 `dist/Teahouse` 直接打包"，不重编前端/后端**——改过代码后必须先 `--frontend`/`--backend`（或干脆无参数全跑）再 `--zip`，否则打出来的是旧代码。发布走下方"发布流程"用无参数全跑即可。`--zip` 额外产出 `dist\Teahouse-<ver>-with-git.zip`（含 git，新用户）与 `dist\Teahouse-<ver>.zip`（不含 git，更新包），版本号单源 = `pyproject.toml`。
...
> **发布流程（用户明确要求"发布"时才执行）**：改代码 → 重 build（`build_release.py` **无参数全跑**，即 check→frontend→backend→verify→zip 重编前后端并出双包；**不要只传 `--zip`**，那只打包不重编）→ 手动 `+1` sw.js 版本 → `git add/commit/push` → 打新 tag（如 `v1.0.1`）→ `gh release create <tag> dist\Teahouse-<ver>-with-git.zip dist\Teahouse-<ver>.zip --title ... --notes ...`。更新说明写清"升级用户下裸名包，整目录替换 `_internal/`/`dist/`/exe，保留 teahouse.yaml 与 data/"。
