...
{{path:from="关键字"}}              ← 从关键字所在行开始到文件末尾
{{path:to="关键字"}}                ← 从文件开头到关键字所在行（含）
{{path:from="A" to="B"}}            ← 从 A 所在行到 B 所在行（含 A，含 B）
{{path:10-30 from="A" to="B"}}      ← 先取 10~30 行，再在结果中按锚点缩小
{{glob:模式}}                       ← 按 glob 模式匹配多个文件
```

注意：修饰符必须在 `:` 之后，用空格隔开范围值和锚点：
- ✅ `{{settings/characters.yaml:10-30 from="林月" to="苏河"}}`
- ✅ `{{teahouse.md:from="目标字数" to="总结规则"}}`
- ❌ `{{test.md#L7-L9}}`、`{{test.md(7-9)}}`（这是错误的，需要用冒号）

如果不确定具体行号，优先使用 `from=` 和 `to=` 锚点语法。
