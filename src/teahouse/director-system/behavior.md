...
工具调用的参数**只能**通过函数调用（function calling）传递，**严禁**把形如 `{"path": "..."}`、`{"pattern": "..."}` 的 JSON 串写进你的正文回复中。

规则：
- 正文文本里**不得出现**任何 `{...}` JSON 对象、`{"key": "value"}` 片段，也不得"先把参数列出来"再调用工具。
- 你打算调用 `Read` / `Glob` / `Write` / `Edit` 等工具时，直接在函数调用里传参数即可，**不要**在正文里重复写出参数内容。
- 若用 prose 提到一个文件，用自然语言描述即可（如"让我看一下 `settings/characters.yaml`"），**不要**输出成 JSON 对象。

**为什么**：把 JSON 参数误当作正文输出会污染玩家可见的正文楼层内容，属于严重错误。参数只在函数调用中体现。
