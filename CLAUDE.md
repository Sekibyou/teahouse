...
> **构建链路（幂等六步，源码见 build/build_release.py）**：`--check` 校验 venv python/pyinstaller/pnpm/`build\mingit_tmp\cmd\git.exe` → `--frontend` pnpm build → `--backend` pyinstaller --clean（COLLECT 自带清 `dist/Teahouse`，故可重复）→ `--assets` 把前端算 dir hash + 压成 `dist.zip`（顶层为 `dist/`）+ 写 `dist.hash`（zip+dir 双 hash，**先算 hash、再打 zip、后写 hash** 确保同源；并对 zip 内部解压复核一次）→ `--verify` 四要素（exe/_internal/git/dist.zip；且复核 dist.hash 的 zip hash 与当前 dist.zip 自洽）→ `--zip` 双包。MinGit 装配源 = 已解压的 `build\mingit_tmp`（不再要求 tests 下 zip）。
...
> **发布流程（用户明确要求"发布"时才执行）**：改代码 → 重 build（`build_release.py` **无参数全跑**，即 check→frontend→backend→assets→verify→zip 重编前后端并出双包；**不要只传 `--zip`**，那只打包不重编）→ 手动 `+1` sw.js 版本 → `git add/commit/push` → 打新 tag（如 `v1.0.1`）→ `gh release create <tag> dist\Teahouse-<ver>-Windows-with-git.zip dist\Teahouse-<ver>-Windows.zip --title ... --notes ...`。更新说明写清"升级用户下裸名包，覆盖 `Teahouse.exe`/`_internal/`/`dist.zip`/`dist.hash`，保留 teahouse.yaml、data/ 与旧 `dist/`（启动时自动对照 hash 复用或重建）；前端在升级后首次启动时自动对齐"。
> **跨架构源码一键包（Termux / aarch64 / 任一 Linux 原生，不经 PyInstaller）**：PyInstaller 产物绑定宿主架构（x86-64 打包在 aarch64 Termux 跑不了，proot 模拟又极慢）。为此 `--zip` 额外产出 **`dist\teahouse-<ver>-source.zip`**（内容：`Teahouse` 引导脚本 + `dist.zip` + `dist.hash` + `VERSION`；版本号 = pyproject 单源），并产出一个**独立 `frontend-dist-<ver>.zip` + `dist.hash`** asset。引导脚本 `build/source/Teahouse`（shebang Python，跨架构）：`git clone --branch v<ver>` 源码 → `uv venv` + `uv pip install -e .`（抓 aarch64 预编译 wheel，关键依赖 pydantic-core/cryptography/uvloop/bcrypt 在 PyPI 均有 cp311-*aarch64 wheel）→ 把包内 `dist.zip`+`dist.hash` 种进工作区 `.teahouse-dist/` 自愈 → `python -m teahouse` 启动。**`./Teahouse --update`** 下载最新 release Source code（**不走 git pull**，防代码库超前 release）+ 最新依赖。源码一键包与 `frontend_install.ensure_frontend()` 的源码态分支（B1 默认自愈）配套：源码运行时不 build 前端、前端产物不进 git（`.teahouse-dist/` 与 `teahouse-frontend/dist` 均已 gitignore），本地 dist.zip 缺失才去 release 拉当前版本 asset。Termux 安装：`build/source/install-termux.sh`（装 git/python/uv → 拉最新一键包 → `./Teahouse`）。发布时把三个新 asset 一并 `gh release upload`：`teahouse-<ver>-source.zip`、`frontend-dist-<ver>.zip`、`dist.hash`。Windows 开发机源码调试不受影响（源码态 dist 已在则直接复用）。
